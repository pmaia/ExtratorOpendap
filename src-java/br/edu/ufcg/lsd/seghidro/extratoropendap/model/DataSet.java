...
public class DataSet {
	 ...
	 *            neste DataSet. O caminho pode ser tanto referente à um arquivo
	 *            netcdf local quando a uma url opendap, referente a um dataset
	 *            acessado remotamente.
	 * @throws ExtratorOpendapException
	 *             Caso haja algum problema no acesso ao dataset indicado pelo
	 *             caminho.
	 */
	public DataSet(String path, Date dataInicial, Date dataFinal)
			throws ExtratorOpendapException {
		this(path);
	...
	public DataSet(String path) throws ExtratorOpendapException {
		if (path == null || path.trim().isEmpty()) {
			throw new IllegalArgumentException(
					"Informe um caminho de dataset válido. Foi passado: '"
							+ path + "'.");
		}
		try {
			gridDs = GridDataset.open(path);
		} catch (IOException e1) {
			throw new ExtratorOpendapException("O dataset '" + path
					+ "' não existe ou não pode ser aberto.", e1);
		}

		if (getTimeVariable() == null) {
			throw new ExtratorOpendapException(
					"O arquivo de origem não possui uma variável 'time' definida.");
		}

		int timeLength = Math.round(getTimeVariable().getSize());
		if (timeLength > Constantes.MAX_TIME_LENGTH) {
			throw new ExtratorOpendapException(
					"O Dataset possui coordenada de tempo muito grande: '"
							+ timeLength + "'. O Máximo permitido é: "
	...
	public String getNome() {
		try {
			...
			if (nome.lastIndexOf(".") != -1) {
				String nomeSemExtencao = nome.substring(0,
						nome.lastIndexOf("."));
	...
	public void defineCoordenadasDeTempo(Date dataInicial, Date dataFinal) {
		...
		// prenche as coordenadas de tempo mantendo um backup com os índices
		// originais
		if (fim - inicio > 0) {
			...
			for (int i = inicio; i <= fim; i++) {
				coordenadasDeTempo[cont] = NetcdfAPIUtil.readTime(
						getTimeVariable(), i);
	...
	public String getNomeModeloOrigem() {
		for (Object o : this.gridDs.getGlobalAttributes()) {
			...
			if (attribute.getName().equalsIgnoreCase(MODEL_TAG)
					&& attribute.isString()) {
	...
	public int getAnoCentral() {
		...
		Object dateRelative = NetcdfAPIUtil.readSingleValue(timeVariable,
				new int[] { 0 });

		DateType data;
		try {
			data = new DateType(dateRelative + " "
					+ timeVariable.getUnitsString(), null, null);
			Date d = data.getDate();
			int anoCentral = Integer.parseInt(formatadorDeDatas.format(d)
					.substring(0, 4));
			return anoCentral;
		} catch (ParseException e) {
			logger.error("Não conseguiu pegar o ano central. Vai retornar 0.",
					e);
			return 0;
		}
	...
	public String getInicioPeriodo() {
		try {
			...
			String temp = nameDataSet.substring(indexInicioPeriodo,
					indexLastHifem).replace("o", "");
			...
			logger.warn("Não conseguiu pegar o início do período. Vai retornar \"\" "
					+ e.getMessage());
			return "";
		}
	}
	...
	public String getFimPeriodo() {
		...
		try {
			...
			String provavelFimPeriodo = nameDataSet.substring(
					indexLastHifem + 1, indexFimPeriodo);
			...
			logger.error("Não conseguiu pegar o fim do período. Vai retornar \"\" "
					+ e.getMessage());
			return "";
		}
	...
	public String getNomeCenarioOrigem() {
		for (Object o : this.gridDs.getGlobalAttributes()) {
			Attribute attribute = (Attribute) o;
			if (attribute.getName().equalsIgnoreCase(SCENARIO_TAG)
					&& attribute.isString()) {
				return attribute.getStringValue();
			}
		}
	...
	public int getIndexFromTime(Date date) {
		for (int i = 0; i < coordenadasDeTempo.length; i++) {
			if (date.equals(coordenadasDeTempo[i])
					|| coordenadasDeTempo[i].after(date)) {
				return i;
			}
		}
	...
	public Date[] getCoordenadasDeTempo() {
		if (coordenadasDeTempo == null) {
			defineCoordenadasDeTempo(this.gridDs.getStartDate(),
					this.gridDs.getEndDate());
		}
	...
	public String toString() {
		return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE)
				.append("Nome da Variável", this.getVariavelFullName())
				.append("Nome do Cenario", this.getNomeCenarioOrigem())
				.append("Período:", this.getPeriodo())
				.append("Nome do Modelo:", this.getNomeModeloOrigem())
				.append("Path:", this.getPath()).toString();
	}
