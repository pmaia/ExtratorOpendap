...
public class FatorDeConversao {
    ...
    public static final FatorDeConversao DEFAULT = new FatorDeConversao(Constantes.SELF, "");
     ...
     * Para converter algo de Kelvin (K) para Célsius (C) a expressão é "self-273".
     */
    ...
    public double converte(double valor) {
		...
		double computeExpression = parser.computeExpression(expressao.replace(Constantes.SELF, valor + ""));
	...
	return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).append("expressao", expressao).append(
		"unidadesAposConversao", unidadesAposConversao).toString();
    }
