...
public class InterpoladorBilinearOPeNDAP implements InterpoladorIF {
    ...
    private static Logger logger = Logger.getLogger(InterpoladorBilinearOPeNDAP.class);

    /*
     * (non-Javadoc)
     * 
     * @see br.edu.ufcg.lsd.seghidro.mudancasClimaticas.extrator.interpolador.InterpoladorIF#interpola(br.edu.ufcg.lsd.seghidro.mudancasClimaticas.extrator.DataSet,
	...
	try {
		...
		if (grid == null) {
		    throw new ExtratorOpendapException("DataSet de origem não possui dados para a variável especificada:" + variavel);
		}
	    } else {
	    ...
	    int y1 = latitudeGrid;
	    double longitude = NetcdfAPIUtil.readSingleValueAsFloat(longitudeVariable, new int[] { longitudeGrid });
	    double latitude = NetcdfAPIUtil.readSingleValueAsFloat(latitudeVariable, new int[] { latitudeGrid });

	    // Determina o raio de abrangência para o valor de longitude
	    // encontrado.
	    double raioDeAbrangenciaLongitude = findRangeValueFromCoordinate(longitudeVariable, longitudeGrid);

	    // Determina o raio de abrangência para o valor de latitude
	    // encontrado.
	    double raioDeAbrangenciaLatitude = findRangeValueFromCoordinate(latitudeVariable, latitudeGrid);
	    ...
	    double xw1 = (coordenadas.getLongitude() - p1x) / (2 * raioDeAbrangenciaLongitude);
	    double xw2 = 1d - xw1;

	    // determina os pesos dos valores de latitude vizinhos.
	    double yw1 = (coordenadas.getLatitude() - p1y) / (2 * raioDeAbrangenciaLatitude);
	    double yw2 = 1d - yw1;
	    ...
	    double f22;
	    if (grid.getRank() == 3) {
		f11 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, y1, x1 }, unicoElemento);
		f12 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, y2, x1 }, unicoElemento);
		f21 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, y1, x2 }, unicoElemento);
		f22 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, y2, x2 }, unicoElemento);
	    } else if (grid.getRank() == 4) {
		f11 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, 0, y1, x1 }, unicoElemento);
		f12 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, 0, y2, x1 }, unicoElemento);
		f21 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, 0, y1, x2 }, unicoElemento);
		f22 = NetcdfAPIUtil.readAsFloat(variable, new int[] { time, 0, y2, x2 }, unicoElemento);
	    } else {
	    ...
	    throw new ExtratorOpendapException("Não pôde efetuar a leitura do DataSet.");
	} catch (InvalidRangeException e) {
	    logger.error("Algoritmo de extração inadequado para o DataSet.", e);
	    throw new ExtratorOpendapException("Algoritmo de extração inadequado para o DataSet.");
	} catch (RuntimeException e) {
	    logger.error("Não conseguiu efetuar a leitura do DataSet. Motivo: ", e);
	    throw new ExtratorOpendapException("Não conseguiu efetuar a leitura do DataSet. Motivo: " + e.getMessage());
	}
    }

    /**
     * Determina o raio de abrangência para o valor de uma data coordenada (<tt>coordinate</tt>).
     * 
     * @param coordinate
     *                A coordenada.
     * @param coordinateIndex
     *                O índice da coordenada no grid.
     * @return o raio de abrangência para o valor de uma data coordenada (<tt>coordinate</tt>).
     * @throws IOException
     * @throws InvalidRangeException
     */
    private double findRangeValueFromCoordinate(CoordinateAxis coordinate, int coordinateIndex) throws IOException, InvalidRangeException {
	Array long_bounds = coordinate.read(new int[] { coordinateIndex - 1 }, new int[] { 2 });
