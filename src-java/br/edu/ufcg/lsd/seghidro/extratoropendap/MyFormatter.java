...
public class MyFormatter extends FormatadorAbstract {
    ...
    public void adicionaExtracao(Coordenadas coordenadas, double valor) {
		...
		if (precipitacao.contains("."))
				precipitacao = precipitacao.substring(0,  precipitacao.indexOf(".") + 2 );
		valor = Double.valueOf(precipitacao);	
    	
    	formatter.format("%19.4f %19.4f    %S %S %S %12.1f            %S" + FIM_DE_LINHA,
				coordenadas.getLongitude(), coordenadas.getLatitude(),
				altitude, dataFormatada, time, valor, hifen);
    }
