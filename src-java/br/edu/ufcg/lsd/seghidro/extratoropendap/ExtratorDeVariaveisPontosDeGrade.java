 ...
 * {@link Extracao}, que podem ser recuperadas em forma de objetos (
 * {@link ExtratorDeVariaveisPontosDeGrade#getListaDeExtracoes()}) ou em em
 * formato String já ({@link ExtratorDeVariaveisPontosDeGrade#getExtracoes()}).
 * 
 * @author Edigley P. Fraga, edigley@lsd.ufcg.edu.br
 * @since 12/02/2009
 * 
 */
public class ExtratorDeVariaveisPontosDeGrade extends
		ExtratorDeVariaveisAbstract {

	private static Logger logger = Logger
			.getLogger(ExtratorDeVariaveisPontosDeGrade.class);

	private Coordenadas coordMinEsquerda;
	private Coordenadas coordMaxDireita;

	public ExtratorDeVariaveisPontosDeGrade(DataSet dataSet, Date dataInicial,
			Date dataFinal, String variavelDeInteresse, Coordenadas cp1,
			Coordenadas cp2) throws ExtratorOpendapException {
	...
	public ExtratorDeVariaveisPontosDeGrade(DataSet dataSet, Date dataInicial,
			Date dataFinal, Coordenadas cp1, Coordenadas cp2)
	...
	public ExtratorDeVariaveisPontosDeGrade(DataSet ds,
			String variavelDeInteresse, Coordenadas cp1, Coordenadas cp2)
			throws ExtratorOpendapException {
	...
	public ExtratorDeVariaveisPontosDeGrade(DataSet ds, Coordenadas cp1,
			Coordenadas cp2) throws ExtratorOpendapException {
		super(ds);
		this.coordMinEsquerda = cp1;
		this.coordMaxDireita = cp2;
	...
	private void extraiPontosDeGrade() throws ExtratorOpendapException {
		try {
			...
			LatLonPointImpl p1 = new LatLonPointImpl(
					coordMinEsquerda.getLatitude(),
					coordMinEsquerda.getLongitude());
			LatLonPointImpl p2 = new LatLonPointImpl(
					coordMaxDireita.getLatitude(),
					coordMaxDireita.getLongitude());
			...
			for (int n = 0; n < dataSet.getCoordenadasDeTempo().length; n++) {
				...
				if (intervaloNaoEhRelevante()
						|| dataEstahNoIntervaloEspecificado(d)) {
					for (int lat = rangeLatitude.first(); lat <= rangeLatitude
						...
						for (int lon = rangeLongitude.first(); lon <= rangeLongitude
							...
							double valor;
							int[] unicoElemento = NetcdfAPIUtil
									.getUniqueShapeForGrid(grid);
							if (grid.getRank() == 3) {
								valor = NetcdfAPIUtil.readAsFloat(
										grid.getVariable(), new int[] { t, lat,
												lon }, unicoElemento);
							} else if (grid.getRank() == 4) {
								valor = NetcdfAPIUtil.readAsFloat(
										grid.getVariable(), new int[] { t, 0,
												lat, lon }, unicoElemento);
							} else {
							...
							Coordenadas coordenadas = new Coordenadas(lati,
									longi, d);
							Extracao extracao = new Extracao(coordenadas,
									valor, fatorDeConversao);
