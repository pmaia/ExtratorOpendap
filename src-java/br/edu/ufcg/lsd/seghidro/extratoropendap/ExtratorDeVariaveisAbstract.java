 ...
 * que podem ser recuperadas em forma de objetos ({@link ExtratorDeVariaveisAbstract#getListaDeExtracoes()})
 * ou em em formato String já ({@link ExtratorDeVariaveisAbstract#getExtracoes()}).
 * 
 * @author Edigley P. Fraga, edigley@lsd.ufcg.edu.br
 * @since 12/02/2009
 * 
 */
public abstract class ExtratorDeVariaveisAbstract implements ExtratorDeVariaveisIF {

    protected static Logger logger = Logger.getLogger(ExtratorDeVariaveisAbstract.class);
     ...
     *                 Caso haja algum problema na leitura do arquivos
     *                 informado.
     */
    public ExtratorDeVariaveisAbstract(DataSet dataSet) throws ExtratorOpendapException {
	this();
	this.dataSet = dataSet;
	this.fatorDeConversao = new FatorDeConversao(Constantes.SELF, dataSet.getUnidade());
    }

    public ExtratorDeVariaveisAbstract(DataSet dataSet, String variavelDeInteresse) throws ExtratorOpendapException {
	this(dataSet);
    ...
    public ExtratorDeVariaveisAbstract(DataSet dataSet, String variavelDeInteresse, Date dataInicial, Date dataFinal) throws ExtratorOpendapException {
	this(dataSet, variavelDeInteresse);
	this.dataInicial = dataInicial;
	this.dataFinal = dataFinal;
    }

    public ExtratorDeVariaveisAbstract(DataSet dataSet, Date dataInicial, Date dataFinal) throws ExtratorOpendapException {
	this(dataSet);
	this.dataInicial = dataInicial;
	this.dataFinal = dataFinal;
    }
	...
	logger.debug("Data esta no intervalo especificado? :" + LocaleUtil.formatadorDeDatas.format(dataInicial) + ".."
		+ LocaleUtil.formatadorDeDatas.format(dataFinal) + ": " + retorno);
	return retorno;
    }
	...
	return new ToStringBuilder(this, ToStringStyle.SHORT_PREFIX_STYLE).append("variavelDeInteresse", variavelDeInteresse)
		.append("dataInicial", dataInicial).append("dataFinal", dataFinal).toString();
    }

    @Override
    public abstract String extraiValoresDeInteresse() throws ExtratorOpendapException;
